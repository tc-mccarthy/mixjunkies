...
;(function($) {
	$.fullwidthAudioPlayer = {version: '1.4.3', author: 'Rafael Dery'};
	...
	jQuery.fn.fullwidthAudioPlayer = function(arg) {
		...
		function _documentTrackHandler() {
			...
			if($elem.jquery >= "1.7"){
				$('body').on('click', '.fap-my-playlist li a, .fap-single-track', _addTrackFromDocument, false);
				$('body').on('click', '.fap-add-playlist', _addTrackFromDocument, false);
			}
			else {
				$('body').delegate('.fap-my-playlist li a, .fap-single-track', 'click', _addTrackFromDocument, false);
				$('body').delegate('.fap-add-playlist', 'click', _addTrackFromDocument, false);
			}
		...
		$.fullwidthAudioPlayer.addTrack = function(trackUrl, title, meta, cover, linkUrl, playIt) {
			...
			if(RegExp('http(s?)://soundcloud').test(trackUrl)) {
		...
		function _createPlaylistTrack(cover, title) {
			...
			function _selectTrackFromPlaylist() {
				var index = $playlistWrapper.children('li').index($(this).parent());
